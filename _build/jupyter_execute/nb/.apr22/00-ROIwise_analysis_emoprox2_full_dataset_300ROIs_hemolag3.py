...
# we assume that as earlier model, this model's performance will also be above chance
...
# Removing `Visual`, and `Default` drops accuracy the most. Note that `Default` with other networks does not drop accuracy. And `Default` on its own also does not bring change.
...
# `Visual`, `Default`, and then `Ventral attention` can be considered important.
...
# Now `Visual`, and `Ventral attention`. `Default` is not at the top.
...
# `Visual`, `Ventral attention`, and `Somato motor`!!

# These combinations complete the search space: ${7 \choose r} \text{ } \forall \text{ } r \in [1, \cdots, 7]$, because ${7 \choose r} = {7 \choose 7-r}$.
