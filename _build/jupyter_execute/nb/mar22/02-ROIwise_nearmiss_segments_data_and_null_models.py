...
X_test = tf.convert_to_tensor(X_test, name='input')
y_test = tf.convert_to_tensor(y_test, name='label')
...
for idx_null in tqdm(np.arange(num_null)):
    ...
    if os.path.exists(null_model_file):
        ...
        null_adv_history = null_adv_model.fit(
            x={'input': X_train, 'label': tf.convert_to_tensor(
                np.random.randint(
                    low=0, high=2, 
                    size=(y_train.shape)).astype(np.float64))}, 
            epochs=args.num_epochs,
            validation_split=args.validation_split,
            batch_size=args.batch_size,
            verbose=1)
